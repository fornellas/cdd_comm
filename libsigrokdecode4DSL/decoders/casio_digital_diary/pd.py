...
class Frame:
    ...
    def get_type_str(self) -> str:
        if self.length == 0x2 and self.type == 0x0 and self.address == 0x200:
        ...
        if (
            ...
            return "color"
        if self.length == 0xA and self.type == 0xF0 and self.address == 0x0:
            return "date"
        if self.length == 0xA and self.type == 0xF4 and self.address == 0x0:
            return "todo-date"
        if self.length == 0x1 and self.type == 0x72 and self.address == 0x0:
            return "todo-priority"
        if self.length == 0x5 and self.type == 0xE4 and self.address == 0x0:
            return "todo-time"
        if self.length == 0x5 and self.type == 0xC4 and self.address == 0x0:
            return "todo-alarm"
        if self.length == 0x20 and self.type == 0x78 and self.address == 0x0:
            return "calendar-date-color-highlight"
        if self.length in [0xB, 0x5] and self.type == 0xE0 and self.address == 0x0:
            return "time"
        if self.length == 0x5 and self.type == 0xC0 and self.address == 0x0:
            return "alarm"
        ...
        if self.length == 0x0 and self.type == 0x0 and self.address == 0x100:
            return "record-end"
        if self.length == 0x0 and self.type == 0x0 and self.address == 0xFF00:
            return "finish"
        return "unknown"
    ...
    def __str__(self) -> str:
        ...
        if type_str == "text":
            return f"Text ({self.address}): " + "".join(
    ...
    def _decode_sender_frame_header_type(self, startsample, endsample, data):
        ...
        if value is not None:
            ...
            self.put(
                ...
                [
                    _get_annotation_index(self.annotations, "frame-header"),
                    ["Type: " + hex(self._frame.type)],
                ],
            )
            self._data_count = self._frame.length
            self._sender_decode_function = self._decode_sender_frame_header_address
        else:
            self._chunk_startsample = startsample
        return True

    def _decode_sender_frame_header_address(self, startsample, endsample, data):
        value = self._decode_hex(data)
        if value is not None:
            if len(self._address_bytes) < 2:
                self._address_bytes.append(value)

            if len(self._address_bytes) == 2:
                self._frame.address = (self._address_bytes[1] << 8) | (
                    self._address_bytes[0] & 0xFF
                )
                self.put(
                    self._chunk_startsample,
                    endsample,
            ...
            if not len(self._address_bytes):
                self._chunk_startsample = startsample
        return True
